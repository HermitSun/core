...
prepare_parsers:
	mkdir -p build
	cd build && git clone --single-branch --depth=1 -b $(parsers-version) https://github.com/crimson-unicorn/parsers

prepare_graphchi:
	mkdir -p build
	cd build && git clone --single-branch --depth=1 -b $(graphchi-version) https://github.com/crimson-unicorn/graphchi-cpp
	cd build/graphchi-cpp && make sdebug

prepare_modeling:
	mkdir -p build
	cd build && git clone --single-branch --depth=1 -b $(modeling-version)  https://github.com/crimson-unicorn/modeling
